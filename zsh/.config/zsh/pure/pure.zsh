...
prompt_pure_preprompt_render() {
	...
	local -ah ps1
    preprompt=${(j. .)preprompt_parts}
    # preprompt_clean=$(print -Pn $preprompt | cat -A | sed 's/\^[^m\]*m//g')
    preprompt_clean=$(print -Pn "$preprompt" | sed $'s,\e\\[[^m]*m,,g')
    preprompt_l=${#preprompt_clean}

	ps1=(
		$preprompt				  # Join parts, space separated.
...
function clear-preprompt-line() {
  cols=$(tput cols)
	print -Pn "\e[$(($preprompt_l / (1 + $cols) + 1))A\r\e[0J%F{blue}${PWD##*/} %F{green}$%f"
  if [[ ! -z $BUFFER ]];then
    print -rn -- " $BUFFER"
  fi
}

function clear-preprompt-and-screen() {
	clear-preprompt-line
	zle clear-screen
}
